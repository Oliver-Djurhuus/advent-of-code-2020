...
validField ["eyr", x] = length x == 4 && 2020 <= read x && read x <= 2030
...
validField ["iyr", x] = length x == 4 && 2010 <= read x && read x <= 2020
validField ["pid", x] = length x == 9 && 9 == length (filter (\y -> isDigit y) x)

isValid2 :: [[String]] -> Bool
isValid2 passport = 0 == length (filter (/=True) (map validField passport))
